...
## 3. 目前狀態（2026-08-05）— **完成上線，電腦 + 手機皆可玩**

**全部完成並實測 OK：** A/B/C/D 四場景與互動、游標視覺系統、字色依背景切換、(URGENCY) 緊急效果、全套音效（電腦 + iOS 每次點擊都響）、GitHub 版控、**Netlify 上線（push 自動部署）**、規格書已同步。

**手機觸控版（已合併 main、手機實測通過）：**
- 方案 B「直接點」：偵測觸控 → `body.touch`；touch 對應（跟隨文字跟手指、輕點觸發熱區、按住馬桶尿尿、拖曳撥水）。沖水改「**點此沖水**」按鈕（桌機仍空白鍵）；直向顯示「請橫置」。
- 手機修過的坑：`svh` 解決橫放被工具列截斷；游標由 `lastFrac` 換算避免與噴發錯位；**音效改純 `<audio>`（見第 4 節）解決 iOS 只響一次/全啞**。

**選配 / 已知取捨（未做，非必要）：**
- `<audio>` 音量上限 1.0：沖水想更大聲需「把 `flush sound.mp3` 檔案本身調響」再覆蓋 push。
- sink 用 `<audio loop>` 可能有輕微接縫；手機體驗可再進一步優化。
